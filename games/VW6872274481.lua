...
run(function()
	...
	insta = vape.Categories.Utility:CreateModule({
...
run(function()
	...
	AutoSkywars = vape.Categories.Minigames:CreateModule({
		Name = 'AutoSkywars',
		Function = function(callback)
			if callback then
				task.spawn(function()
			...
			if callback then
				task.spawn(function()
					repeat
						...
						game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("HangGliderUse"):FireServer(unpack(args))						
					until (not insta.Enabled)
				end)
			end
		end, 
